...
impl BackupService {
    ...
    ///
    /// Creates a safety backup first, then closes the database, replaces
    /// its data file with the backup, and reopens it.
    pub async fn restore_database(
        ...
        let safety_entry = entry.clone();
        let safety_path_clone = safety_path.clone();
        tokio::task::spawn_blocking(move || safety_entry.db.save(&safety_path_clone))
            .await
            .map_err(|e| ServiceError::Internal(e.to_string()))?
            .map_err(|e| ServiceError::Internal(format!("safety backup failed: {e}")))?;

        // Drop our reference before removing from registry
        let metadata = entry.metadata.clone();
        drop(entry);

        // Remove database from registry
        let removed = databases.remove(db_name);
        if removed.is_none() {
            return Err(ServiceError::Internal(
                "database disappeared during restore".to_string(),
            ));
        }
        let (_, old_entry) = removed.unwrap();

        // Close the engine
        if let Err(e) = old_entry.db.close() {
        ...
        //
        // The backup is a `.grafeo` single-file snapshot. The live database uses
        // directory-based storage (`grafeo.db` dir with WAL). We open the backup,
        // remove the old data, and save from the backup into the original path.
        let db_dir = data_dir.join(db_name);
        let db_file = db_dir.join("grafeo.db");

        // Remove old database files/dirs
        if db_file.exists() {
            if db_file.is_dir() {
        ...
        // Open backup as source and save to the persistent path
        ...
        let open_result = tokio::task::spawn_blocking(move || -> Result<GrafeoDB, String> {
            ...
            backup_db.close().ok();
            // Now open the restored database
        ...
        match open_result {
            ...
            Err(e) => {
                ...
                tracing::error!(
                    database = %db_name,
                    error = %e,
                    "Failed to restore, attempting recovery from safety backup"
                );
                ...
                if let Ok(db) = GrafeoDB::open(db_file.to_str().unwrap()) {
                    databases.insert(
                        db_name.to_string(),
                        Arc::new(DatabaseEntry {
                            db: Arc::new(db),
